...
class LoginView(APIView):
    def post(self,request):
        ...
        if user :
            ...
            return Response({'error':'Inavalid Credentials'},status=status.HTTP_401_UNAUTHORIZED)
...
class UserBookingView(APIView):
    ...
    def get(self,request,user_id):
        if request.user.id != user_id:
            return Response({'erro':'Unauthorized'},status=status.HTTP_401_UNAUTHORIZED)
